...
- **`CHANGELOG.md` Requirement**: Every single Git commit MUST include an entry in `CHANGELOG.md` with an explicit version and date tag in the format `## [vX.Y.Z] - Title (YYYY-MM-DD)` (e.g. `## [v0.8.0] - Crawl4AI Integration & Advanced Extraction (2026-08-28)`). **NEVER use `[Unreleased]` or date-less untagged placeholders**. Synchronize `version` in `package.json` whenever bumping versions.
